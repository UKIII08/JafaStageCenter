...
class Settings(db.Model):
    ...
    minor_style = db.Column(db.String(10), default='lowercase')

def get_notation():
    s = Settings.query.first()
    notation = s.chord_notation if s and s.chord_notation else 'international'
    minor_style = s.minor_style if s and s.minor_style else 'lowercase'
    return notation, minor_style
...
def check_db_schema():
    with app.app_context():
        try:
            ...
            if 'minor_style' not in settings_columns:
                with db.engine.connect() as conn:
                    conn.execute(text("ALTER TABLE settings ADD COLUMN minor_style VARCHAR(10) DEFAULT 'lowercase'"))
                    conn.commit()
...
def normalize_song_chords(content, input_notation='international'):
    def replace_chord(m):
        ...
        if '/' in cleaned:
            ...
            if is_valid_chord(parts[0]):
                normalized_parts = [normalize_chord(p, input_notation) for p in cleaned.split('/')]
                return '[' + '/'.join(normalized_parts) + ']'
            return '[' + cleaned + ']'
        if not is_valid_chord(cleaned):
            return '[' + cleaned + ']'
        return '[' + normalize_chord(cleaned, input_notation) + ']'
        ...
        try:
            with app.test_request_context(base_url='http://127.0.0.1:5000'):
                ...
                for item in setlist_data:
                    ...
                    notation, minor_style = get_notation()
                    _, _, text_print = process_song(item.get('content', ''), t_val, notation=notation, minor_style=minor_style)
...
def _format_chord_for_display(chord_str, notation='international', minor_style='lowercase'):
    if not chord_str:
        return chord_str
    chord_str = clean_chord(chord_str)
    if '/' in chord_str:
        parts = chord_str.split('/')
        return '/'.join(_format_chord_for_display(p, notation, minor_style) for p in parts)
    if not is_valid_chord(chord_str):
        return chord_str
    m = CHORD_ROOT_RE.match(chord_str)
    if not m:
...
def process_song(text, transpose_amount=0, notation='international', minor_style='lowercase'):
    if not text: return "", "", ""
    text = text.strip()
    if transpose_amount != 0:
        text = re.sub(r'\[(.*?)\]', lambda m: transpose_chord(m, transpose_amount, notation), text)
    def format_display(m):
        return '[' + _format_chord_for_display(m.group(1), notation, minor_style) + ']'
    ...
    for i, token in enumerate(tokens):
        if token.startswith('[') and token.endswith(']'):
            ...
            elif is_chord_mid_word(i, tokens):
...
def add_song():
    ...
    if content:
        content = normalize_song_chords(content, input_notation)
    if key:
        key = normalize_chord(key, input_notation)
...
def import_songs():
    ...
    def save_or_update(song_title, song_content, song_key='', song_bpm=0):
        ...
        song_content = normalize_song_chords(song_content, input_notation)
        if song_key:
            song_key = normalize_chord(song_key, input_notation)
...
def edit_song(id):
    ...
    input_notation = request.form.get('input_notation', 'international')
    if song.content:
        song.content = normalize_song_chords(song.content, input_notation)
    song.key = request.form.get('key')
    if song.key:
        song.key = normalize_chord(song.key, input_notation)
    bpm = request.form.get('bpm')
...
def update_settings():
    ...
    settings.minor_style = request.form.get('minor_style', 'lowercase')
...
def reset_settings():
    ...
    settings.minor_style = 'lowercase'
    db.session.commit()
    bg_path = os.path.join(app.static_folder, 'background.png')
...
def print_setlist():
    ...
    songs_to_print = []
    for item in data:
        _, _, text_print = process_song(item['content'], int(item['transpose']), notation=notation, minor_style=minor_style)
...
def send_text():
    ...
    people_html, band_html, _ = process_song(raw_text, shift, notation=notation, minor_style=minor_style)
    _, band_next_html, _ = process_song(data.get('next_text', ''), next_shift, notation=notation, minor_style=minor_style)
