...
// Jawna konwersja formatu "akordy nad tekstem" (Ultimate Guitar itp.) na
// [akord]tekst - uruchamiana PRZYCISKIEM w modalu, żeby użytkownik świadomie
// decydował o konwersji i od razu widział jej wynik w polu.
function convertPastedFormat(textareaId, statusId, keyInputId) {
...
function saveBackupDesktop() {
    if (window.pywebview) {
        ...
        window.location.href = (window.JAFA_BASE || "") + "/export_songs";
    }
}
...
(function() {
    ...
    const ONBOARDING_STEPS = [
        ...
        // ── Phase 3: Band Member View ──
        {
            ...
            afterInteract: function() {
                window.open(window.JAFA_BASE ? window.JAFA_BASE.replace('/studio','/live/band') + '?tour=1' : '/band_member?tour=1', '_blank');
            }
        },
